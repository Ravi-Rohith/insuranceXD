...
struct policy {
    string name;
    string ipfsHash;
    // mapping(address => uint256) policyNumber;
    bool isActive;
    uint startDate;
    uint endDate;
...
contract PoliceReportStorage {
    ...
    function addPoliceReport(
        ...
        require(
            policeReports[ipfsHash].maxClaimAmount == 0,
    ...
    function getPoliceReport(
        string memory ipfsHash
    ) public view returns (reportResult memory) {
        require(
            policeReports[ipfsHash].maxClaimAmount != 0,
            "Report with this ID does not exist"
        );
        return policeReports[ipfsHash];
    }

    // Function to retrieve the hashed value of a police report by its ID
    function updatePoliceReport(
        string memory ipfsHash,
        bool isPassed,
        uint256 maxClaimAmount
    ) public {
...
contract TrustedInsurance {
    ...
    function validateClaim(
        ...
        try policeReportStorage.getPoliceReport(_reportCode) returns (
            reportResult memory result
        ) {
            targetReport = result;
        } catch {
        ...
        if (
            ...
            block.timestamp < targetPolicy.endDate &&
            block.timestamp > targetPolicy.startDate
        ) {
